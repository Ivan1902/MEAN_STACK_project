...
export class ModeratorComponent implements OnInit {
  ...
  pratioci: Array<String>;
  porukaNaziv: String;
  poruka3: String;
  flag5: boolean;
  checkboxNiz: boolean[];
  knjigeZaOdobravanje: Knjiga[];
  sviZanrovi: Zanr[];
  datum: Date;
  poruka5: String;
  ...
  ngOnInit(): void {
    ...
    this.pratioci = this.korisnik.pratioci;
    this.checkboxNiz = [];
    ...
    this.servis.dohvatiSveKnjige().subscribe(tmp=>{
      this.sveKnjige = tmp;
      this.knjigeZaOdobravanje = [];

      this.sveKnjige.forEach(knjiga=>{
        if(knjiga.odobrena == false){
          this.knjigeZaOdobravanje.push(knjiga);
        }
      })
  ...
  napravi(){
    ...
    let status;
    let tip;
    this.ucesniciDesavanja = [];
    this.servis.dohvatiSveKorisnike().subscribe((tmp:User[])=>{
      tmp.forEach(korisnik=>{
        if(korisnik.username != this.korisnik.username){
          this.ucesniciDesavanja.push(korisnik.username);
        }
  ...
  odobri(knjiga){
    ...
    var i = this.knjigeZaOdobravanje.indexOf(knjiga);
    this.knjigeZaOdobravanje.splice(i,1);
    this.servis.odobriKnjigu(knjiga).subscribe(tmp=>{

    })
  }

  odobri1(zahtevZaDesavanje){
    var tmp: Desavanje;
    this.svaDesavanja.forEach(desavanje=>{
